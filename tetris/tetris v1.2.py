...
class CLS_AI(object):
    ...
    def score_situation(self, grid):
        ...
        for i in range(len(probeList)):
            probeList[i] += 1
        columns=0
        for i in range(len(probeList)-2):
            if probeList[i+1]-probeList[i]>=4 and probeList[i+1]-probeList[i+2]>=4:
                columns+=1
        cnt = 0
        ...
        try:
            return - ((cnt - score)*1000) - (max(probeList)-min(probeList))*30 -(columns-1)*50  # /((max(probeList)-min(probeList))/2)
