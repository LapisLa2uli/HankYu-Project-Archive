...
class CLS_Framework(object):
    ...
    def completeScene(self, stat):
        ...
        while True:
            ...
            if vFlag == 0:
                ...
                if tL > 1:
                    break
                self.wPage = pygame.Rect(0, 0, SCREEN_W * 3 * (1 - tL) // 5, SCREEN_H * 3 * (1 - tL) // 5)
                self.wPage.center = (SCREEN_W // 2, SCREEN_H // 2)
                self.screen.blit(bg_copy, animation_area)
                pygame.draw.rect(self.screen, (245, 245, 195), self.wPage)
            for event in pygame.event.get():
        ...
        while True:
            ...
            if vFlag == 0:
                if tK < 1:
                    ...
                    self.screen.blit(self.font2.render(f"The word was", True, (50, 50, 150)),
                             ((SCREEN_W - self.font2.size(f"The word was")[0]) // 2, SCREEN_H * 3 // 8))
                    self.screen.blit(self.font2.render(self.prevWord, True, (20, 20, 100)),
                             ((SCREEN_W - self.font2.size(self.prevWord)[0]) // 2, SCREEN_H * 7 // 17))
                    self.screen.blit(self.font3.render("Press 'Enter' to continue", True, (0, 0, 0)),
                             ((SCREEN_W - self.font3.size("Press 'Enter' to continue")[0]) // 2, SCREEN_H * 9 // 17))
                    for event in pygame.event.get():
                        if event.type == pygame.QUIT:
    ...
    def megabomb_page(self):
        ...
        while True:
            ...
            if wFlag == 0:
                if tK < 1:
                    mgbF1 = pygame.transform.scale(self.mgb_frame, (max(1, SCREEN_W * 3 * tK // 4), max(1, SCREEN_H * tK // 2)))
                    ...
                    pygame.draw.rect(self.screen, (255, 255 * (5 - tS) / 5, 0), (SCREEN_W // 6, SCREEN_H // 3 - 3, SCREEN_W * 2 * (1 - tS / 5) // 3, 6))
                    ...
                    self.screen.blit(self.font4.render(f"Enter the word in 5 seconds:", True, (255, 255, 255)),
                             ((SCREEN_W - self.font4.size(f"Enter the word in 5 seconds:")[0]) // 2, SCREEN_H * 3 // 7))
                    self.screen.blit(self.font.render(' '.join(cList), True, (213, 225, 255)), (SCREEN_W // 6 + 40, SCREEN_H * 4 // 7 - self.font.size('G')[1] // 2))
                    pygame.display.update()
                    for event in pygame.event.get():
                        if event.type == pygame.QUIT:
                            pygame.quit()
                ...
                mgbF2 = pygame.transform.scale(self.mgb_frame, (max(1, SCREEN_W * 3 * (1 - tL) // 4), max(1, SCREEN_H * (1 - tL) // 2)))
                mgbF2.set_colorkey((255, 255, 255))
                self.mgbF_rect = mgbF2.get_rect()
                self.mgbF_rect.center = (SCREEN_W // 2, SCREEN_H // 2)
                self.screen.blit(bg_copy, animation_area)
                self.screen.blit(mgbF2, self.mgbF_rect)
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
                    sys.exit()
