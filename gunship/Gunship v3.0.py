...
class CLS_explosion(object):
    def __init__(self,x,y,scale,duration):
        self.x,self.y=x,y
        self.pic=pygame.image.load('explosion.jpeg')
    ...
    def bullet_do(self):
        ...
        for i in range(len(self.z10.bulletList)):
            if i >= len(self.z10.bulletList):
                break
            if self.z10.bulletList[i].x > SCREEN_W or self.z10.bulletList[i].y < SPACE_UP or self.z10.bulletList[
                i].y > SPACE_DOWN:
